...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (isset($_FILES['foto_produk']) && $_FILES['foto_produk']['error'] == 0) {
		...
		$target_dir = "assets/pic/"; // Tanpa ../
		$target_file = $target_dir . basename($foto_produk);

		// Cek apakah direktori ada, jika tidak, buat direktori
		if (!is_dir($target_dir)) {
			mkdir($target_dir, 0755, true); // Buat folder dengan izin baca dan tulis
		}

		// Pindahkan file ke direktori tujuan
		if (move_uploaded_file($_FILES['foto_produk']['tmp_name'], $target_file)) {
			// Update data produk dengan gambar baru
			$update_query = "UPDATE tb_produk SET nama_produk='$nama_produk', id_kategori='$id_kategori', stok='$stok', harga='$harga', diskon='$diskon', deskripsi='$deskripsi', foto_produk='$foto_produk' WHERE id_produk=$id_produk";
		} else {
		...
		$update_query = "UPDATE tb_produk SET nama_produk='$nama_produk', id_kategori='$id_kategori', stok='$stok', harga='$harga', diskon='$diskon', deskripsi='$deskripsi' WHERE id_produk=$id_produk";
	}
								...
								<div class="row">
									...
									<div class="col-lg-6 mb-3">
										<div class="form-group">
											...
											<input type="text" name="nama_produk" class="form-control"
												...
												value="<?php echo htmlspecialchars($row['nama_produk']); ?>" required>
										</div>
									</div>
									...
									<div class="col-lg-6 mb-3">
										<div class="form-group">
											...
											<input type="number" name="diskon" class="form-control" step="0.01" min="0"
												max="100" placeholder="Masukkan diskon"
												value="<?php echo htmlspecialchars($row['diskon']); ?>">
										</div>
									</div>
									...
									<div class="col-lg-12 mb-3">
										<div class="form-group">
											...
											<textarea name="deskripsi" class="form-control" style="height: 200px;"
												...
												required><?php echo htmlspecialchars($row['deskripsi']); ?></textarea>
										</div>
									</div>
									...
									<div class="col-lg-12 mb-3">
										<div class="form-group">
											...
											<img id="preview"
												src="../assets/pic/<?php echo htmlspecialchars($row['foto_produk']); ?>"
												alt="product" style="width: 100px; height: auto; margin-top: 10px;">
											<p id="filename"><?php echo htmlspecialchars($row['foto_produk']); ?>
											</p>
										</div>
									</div>
	...
	<script>
		function previewImage() {
			...
			if (file) {
				...
				// Reset gambar jika tidak ada file yang dipilih
				preview.src = "../assets/pic/<?php echo htmlspecialchars($row['foto_produk']); ?>";
				filename.textContent = "<?php echo htmlspecialchars($row['foto_produk']); ?>";
			}
		}
	</script>
