<?php
include 'includes/db.php'; // Pastikan path ke file koneksi database sudah benar

// Query untuk mengambil data pengguna dari tb_user
...
<body>
                            ...
                            <table id="example3" class="display" style="min-width: 845px">
                                ...
                                <tbody>
                                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                        <tr data-id="<?php echo htmlspecialchars($row['id_user']); ?>">
                                            <td><?php echo htmlspecialchars($row['username']); ?></td>
                                            <td><?php echo htmlspecialchars($row['email']); ?></td>
                                            <td><?php echo htmlspecialchars($row['alamat']); ?></td>
                                            <td><?php echo htmlspecialchars($row['nomor_telepon']); ?></td>
                                            <td><?php echo htmlspecialchars(date('d-m-Y', strtotime($row['created_at']))); ?>
                                            </td>
                                            <td>
                                                <div class="d-flex">

                                                    <a href="javascript:void(0);"
                                                        onclick="confirmDeleteUser(<?php echo $row['id_user']; ?>)"
    ...
    ***********************************-->
    ...
    <script>
        // Fungsi untuk konfirmasi penghapusan user
        ...
        // Fungsi untuk menghapus user melalui AJAX
        function deleteUser(userId) {
                ...
                if (xhr.status == 200) {
                    ...
                    if (xhr.responseText.trim() === 'success') {
                        Swal.fire({
                            ...
                            // Hapus baris user dari tabel setelah berhasil dihapus
                            document.querySelector(`tr[data-id='${userId}']`).remove();
                        });
                    } else {
                        Swal.fire({
                            title: 'Error!',
                            text: 'Terjadi kesalahan saat menghapus user: ' + xhr.responseText,
                            icon: 'error',
                            confirmButtonText: 'OK'
                        });
                    }
            ...
            // Mengirim permintaan dengan ID user yang akan dihapus
            xhr.send("id_user=" + userId);
        }
